...
router.put('/unlike/:id', auth, async (req, res) => {
  try {
    ...
    console.error(error.message)
    res.status(500).send({ message: 'Problem unliking post.' })
  }
})
...
router.post('/comment/:id', [
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(error.message)
      res.status(500).send('Try commenting again later.')
    }
  }
])
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
  try {
    ...
    const removeIndex = post.comments
      .map(comment => comments.user.toString())
      .indexOf(req.user.id)
    ...
    console.error(error.message)
    res.status(500).send('There was a problem deleting this comment.')
  }
})
